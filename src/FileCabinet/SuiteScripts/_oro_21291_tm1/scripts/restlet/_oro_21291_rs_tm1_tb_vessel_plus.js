    ...
    (mavenchecks, query, runtime, dateUtil) => {
        ...
        const runQuery = (request, scriptParameters) => {
            ...
            sqlResult = query.runSuiteQL({
                ...
                return {
                    ...
                    tranCurrency: e.trancurrency,
                    tranAmount: e.amount,
                    subCurrency: e.currency,
                    subAmount: e.amount * e.exchangerate,
                    tourCode: null,
        ...
        const validateParameters = (requestParameters, scriptParameters) => {
            ...
            if (scriptParameters.logLevel === "DEBUG") {
                ...
                log.debug("Test 5: validateLocs", validateGenericParameter(requestParameters, "vessel"));
            }
            ...
            let queryParameters = {
                ...
                loc: validateGenericParameter(requestParameters, "vessel"),
            ...
            switch (query) {
                case "tb":
                    return "SELECT " +
                        ...
                        "trnCurrency.symbol as tranCurrency, " +
                        "trn.exchangerate as exchangeRate, " +
                        "gl.amount, " +
                        "trn.id, " +
                ...
                case "tb_balances":
                    sql = "SELECT " +
                        ...
                        "LEFT JOIN currency as trnCurrency ON trn.currency = trnCurrency.id " +
                        "LEFT JOIN department AS dpt ON line.department = dpt.id " +
                        "LEFT JOIN customrecord_cseg_oro_vessel AS loc ON line.cseg_oro_vessel = loc.id " +
                        "WHERE trn.posting = 'T' " +
                        // "AND acct.custrecord_oro_tourshipcost = 'T' " +
                    ...
                    "ORDER BY subsidiary, account_number, department, vessel;";
                    return sql;
                    break;
                case "mov":
                case "tb_movements":
                case "trialbalance":
                    ...
                    sql += " GROUP BY " +
                        ...
                        "sub.custrecord_oro_sagecompany "   // TODO: Why is "+" missing?
                    "HAVING SUM(TO_NUMBER(gl.amount)) != 0 " +
                    "ORDER BY entity, fiscalYear, fiscalPeriod, account, department, vessel;";
                    return sql;
                    break;
                default:
