 ...
 */
define(['N/record', 'N/search', 'N/runtime', 'N/render', 'N/email', 'SuiteScripts/_Libraries/_lib.js', 'N/format'],
    /**
     * @param{record} record
     * @param{search} search
     ...
     * @param{format} format
     */
    (record, search, runtime, render, email, _lib, format) => {
        ...
        * */
        ...
        function constructEmailBody(paymentId) {
            ...
            let totalAmount = rec.getValue('total');
            const currency = rec.getText('currency');
            totalAmount = _lib.formatNumber(totalAmount, 2, ',');
            ...
            for (let i = 0; i < iLineCount; i++) {
                ...
                let pmtAmount = rec.getCurrentSublistValue({
                    sublistId: 'apply',
                    fieldId: 'amount'
                });
                pmtAmount = _lib.formatNumber(pmtAmount, 2, ',');
        ...
        const map = (mapContext) => {
            try {
                ...
                if (paymentId && emailAddr) {
                    const senderId = 21910; // Scenic Tours Employee (no-reply@scenic.eu)
                    email.send({
                        author: senderId,
